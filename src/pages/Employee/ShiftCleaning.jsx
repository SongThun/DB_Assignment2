import React from 'react'
import Navbar from '../../components/Navbar.jsx'
import Sidebar from '../../components/Sidebar.jsx';
import Card from '../../components/Card.jsx';
import '../../components/Table.css'
import Table from '../../components/Table.jsx';
import axios from 'axios';
import moment from 'moment';

const ShiftCleaning = () => {
  axios.defaults.withCredentials=true;
  
  const main_http = 'http://localhost:8080/employee/cleaner_works_on/';
  const constraints = {
    shift_date: {
      type: "date",
      required: true
    },
    start_time:{
      type: "time",
      required: true,
      end: "22:00"
    },
    end_time:{
      type: "time",
      required: true
    },
    receptionist_id:{
      type: "text",
      required: true
    }
  }

  const feedback = {
    staff_id: "ID must have 8 characters, first 3 characters must be uppercase letters.\nEx: RCT00001",
    ssn: "SSN must be a valid 12-digit social security number",
    email: "Please provide a valid email"
  }

  const SubmitAdd = (values, reloadData) => {
    axios.post(main_http, values)
    .then(res => {
      if (res.data.err) {
        const msg = res.data.err.sqlMessage;
        console.log(msg)
        if (msg.includes('shift.PRIMARY'))
          alert('Duplicate entry for Primary key!');
        if (msg.includes('receptionist_id_shift')) {
          alert('Invalid Receptionist ID!');
        }
        if (msg.includes('time_shift')){
          alert('Time in shift not valid, please check again!');
        }
      }
      else reloadData(res.data,true)
    })
    .catch(err => console.log(err));
  }
      
  const SubmitEdit = (pk, values, reloadData) => {
    let params = '';
    for (const key in pk) {
      params += `${pk[key]}/`;
    }
    axios.put(main_http + params, values)
    .then(res => {
      console.log(res);
      if (res.data.err) {
        const msg = res.data.err.sqlMessage;
        console.log(msg)
        if (msg.includes('Duplicate entry'))
          alert('Duplicate entry for staff_id (primary key)');
      }
      else reloadData(res.data,false)
    })
    .catch(err => console.log(err));
  };
  const header=['Date','Start time','End time', 'CleaningS ID','Area']
  const labels=['Date','Start time','End time', 'CleaningS ID','Area']
  const sortlabels=['Date','Start time','End time', 'CleaningS ID','Area']

  return (
    <div className="container-fluid min-vh-100">
        <div className="row">
				
						<Sidebar active_item="Employee"/>
					

					<div className="col background1 container-fluid">
						<div className="sticky-top border-bottom mb-4 container-fluid">
              <Navbar href="/employee" goBack="true"/>
            </div>

						<div>
              <Table 
                table_size="sm"
                table="Shift of Cleaning Staff"
                sortlabels={sortlabels}
                labels={labels}
                constraints={constraints}
                feedback={feedback}
                main_http={main_http}
                submitEdit={SubmitEdit}
                submitAdd={SubmitAdd}
                header={header}
                crud="true"
              />
						</div>
					</div>
        </div>
    </div>

  )
}

export default ShiftCleaning